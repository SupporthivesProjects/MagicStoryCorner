...
class StoryBookBuilder:
    def __init__(self, user, input_data):
        ...
        config = Configuration.objects.filter(type='user', is_active=True).first()
        self.CHARACTERS_PER_PAGE = config.chars_per_page if config else 400
        self.MAX_PAGES_LIMIT = config.max_pages if config else 25
        self.IMAGE_RETRIES = config.img_retries if config else 1
        self.TTS_RETRIES = config.tts_retries if config else 1
        self.MAX_WORKERS = config.workers if config else 10
        self.CHARACTER_CONSISTENCY_COUNT = config.consistency_checks if config else 4
        self.STORY_GENERATION_MAX_RETRIES = config.story_retries if config else 1
    ...
    def refine_character_description(self, character_name="", character_desc="", char_type="main"):
        if not self.openai_client:
            Log.objects.create(
                ...
                type="error", 
                message=f"User {self.user.id} - Cannot refine character description"
            )
        ...
        try:
            ...
            Log.objects.create(
                ...
                type="error",
                message=f"User {self.user.id} - Error: {str(e)}. Using original description."
            )
            return character_desc
    ...
    def extract_characters_from_story(self, story_text):
        ...
        try:
            ...
            Log.objects.create(
                ...
                type="warning",
                message=f"User {self.user.id} - Error: {str(e)}. Continuing without supporting characters."
            )
    ...
    def generate_story(self):
        if not self.openai_client:
            Log.objects.create(
                title="Missing OpenAI API key", 
                type="error", 
                message=f"User {self.user.id} - no API key configured"
            )
            return None, None
            ...
            try:
                ...
                if attempt < MAX_RETRIES - 1:
                    ...
                    Log.objects.create(
                        ...
                        type="error", 
                        message=f"User {self.user.id} - All {MAX_RETRIES} attempts failed. Final error: {str(e)}"
                    )
    ...
    def generate_image_gemini(self, prompt):
        if not self.genai_client:
            Log.objects.create(
                ...
                type="error", 
                message=f"User {self.user.id} - Cannot generate image without Gemini API key"
            )
            return None
        ...
        for attempt in range(self.IMAGE_RETRIES):
            try:
                response = self.genai_client.models.generate_content(
                    model=self.gemini_name or 'gemini-2.5-flash-image',
                    contents=prompt
                )
                ...
                if "safety" in error_message or "policy" in error_message or "blocked" in error_message:
                    if attempt == self.IMAGE_RETRIES - 1:
                        Log.objects.create(
                            ...
                            type="error", 
                            message=f"User {self.user.id} - {str(e)}"
                        )
                else:
                    if attempt == self.IMAGE_RETRIES - 1:
                        Log.objects.create(
                            ...
                            type="error", 
                            message=f"User {self.user.id} - All {self.IMAGE_RETRIES} attempts failed - {str(e)}"
                        )
                ...
                if "safety" in error_msg or "policy" in error_msg or "blocked" in error_msg:
                    ...
                    if attempt == self.IMAGE_RETRIES - 1:
                        Log.objects.create(
                            ...
                            type="error", 
                            message=f"User {self.user.id} - All attempts failed with {len(reference_images)} references"
                        )
    ...
    def generate_character_reference(self, char_type='main', char_name='', char_desc=''):
        ...
        if image_file:
            ...
            images_dir = f'media/stories/books/{self.current_book_id}/images'
    ...
    def generate_cover_image(self, title, character_desc, imagestyle_content):
        ...
        if not character_keys:
            Log.objects.create(
                ...
                type="error",
                message=f"User {self.user.id} - Cannot generate cover without references"
            )
            return None, None
        ...
        if not reference_images:
            Log.objects.create(
                title="Cover generation failed",
                type="error",
                message=f"User {self.user.id} - No valid image data found"
            )
            return None, None
                ...
                if "safety" not in error_msg and "policy" not in error_msg and "blocked" not in error_msg:
                    if attempt == self.IMAGE_RETRIES - 1:
                        Log.objects.create(
                            ...
                            type="error",
                            message=f"User {self.user.id} - All attempts failed: {str(e)}"
                        )
    ...
    def generate_tts(self, text, voice_id):
        ...
        if not self.elevenlabs_key:
            Log.objects.create(
                title="TTS skipped", 
                type="warning", 
                message=f"User {self.user.id} - ElevenLabs key missing"
            )
            return None
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if attempt == self.TTS_RETRIES - 1:
                        Log.objects.create(
                            ...
                            type="error",
                            message=f"User {self.user.id} - All attempts failed with status {resp.status_code}"
                        )
                ...
                if attempt == self.TTS_RETRIES - 1:
                    Log.objects.create(
                        ...
                        type="error",
                        message=f"User {self.user.id} - All {self.TTS_RETRIES} attempts failed - {str(e)}"
                    )
    ...
    def generate_unique_slug(self, title):
        ...
        slug = base_slug
        counter = 1
        while StoryBook.objects.filter(slug=slug).exists():
    ...
    def save_book(self, story_text=None, title=None):
        if not story_text:
            ...
            if not story_text:
                Log.objects.create(
                    ...
                    type="error",
                    message=f"User {self.user.id} - Story generation failed"
                )
                return None
        ...
        if not pages_text:
            Log.objects.create(
                title="Book generation aborted",
                type="error",
                message=f"User {self.user.id} - Story division failed"
            )
            return None
        
        slug = self.generate_unique_slug(title)
        
        book = StoryBook.objects.create(
            user=self.user,
            title=title,
            slug=slug,
            prompt=self.build_prompt(),
        ...
        os.makedirs(f'media/stories/books/{book.pk}/images', exist_ok=True)
        ...
        try:
            ...
            if not main_char_result:
                Log.objects.create(
                    ...
                    type="error",
                    message=f"User {self.user.id} - Cannot proceed without main character"
                )
                book.status = "failed"
                book.save()
                return None
            ...
            if not self.character_references.get('main', {}).get('image'):
                Log.objects.create(
                    ...
                    type="error",
                    message=f"User {self.user.id} - Reference generated but not in memory"
                )
                book.status = "failed"
                book.save()
                return None
                                ...
                                try:
                                    ...
                                    if retry == self.IMAGE_RETRIES - 2:
                                        Log.objects.create(
                                            ...
                                            type="error",
                                            message=f"User {self.user.id} - Page {page_num} could not be generated after {self.IMAGE_RETRIES - 1} retries"
                                        )
                        else:
                            Log.objects.create(
                                title=f"Page {page_num} failed",
                                type="error",
                                message=f"User {self.user.id} - Page {page_num} could not be generated"
                            )
            ...
            for i in sorted(page_results.keys()):
                ...
                pages_to_create.append(StoryBookPage(
                    book=book,
                    page=i,
                    text=page_text,
                    image=image_file,
            ...
            if pages_to_create:
                with transaction.atomic():
                    StoryBookPage.objects.bulk_create(pages_to_create)
            ...
            book.status = "failed"
            book.save()
            Log.objects.create(title="Book generation failed", type="error", message=f"User {self.user.id} - Error generating book {book.id}: {str(e)}")
            raise